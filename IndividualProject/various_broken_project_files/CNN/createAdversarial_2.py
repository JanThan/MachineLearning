...
def gradient_wrt_input(image, label):
...
def generate_adversarial(x,y):
  ...
  for i in range(epoch):
    pertubation = gradient_wrt_input(x_[0].reshape((1, 28, 28, 1)), label).numpy()
    
    x_ = x_ + pertubation*learning_rate
    
    # calculate euclidian distance
    dist = np.linalg.norm((x.flatten()-x_.flatten()), ord=2)
  ...
  adv = x_.reshape((1,28,28,1))

  return adv

grad = gradient_wrt_input(x0,y0)
print(np.amax(grad))

adv_image = generate_adversarial(x0,y0)
show_data_example(0,adv_image,y0)
  

## Assess base model on adversarial data vs before adversarial
#print("Base accuracy on normal images:", CNN_model.evaluate(x=x0, y=y0, verbose=0))
#print("Base accuracy on adversarial images:", CNN_model.evaluate(x=adv_image, y=y0, verbose=0))

predict = CNN_model.predict(np.expand_dims(adv_image[0,:,:,:],axis=0))
print(predict)
